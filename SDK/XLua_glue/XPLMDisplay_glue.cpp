#ifndef _Glua_XPLMDisplay_h_
#define _Glua_XPLMDisplay_h_

/*
 * This file is auto-generated by header_parser, part of the SDK. Do not
 * hand-edit!
 *
 */

/***************************************************************************
 * XPLMDisplay
 ***************************************************************************/

#include "XPLMDefs.h"

// We need the XPLM_DEPRECATED marker because Lua is interpreted - old Lua scripts will always use the latest SDK.
#define XPLM_DEPRECATED
#include "XPLMDisplay.h"
#undef XPLM_DEPRECATED

#include "../xpfuncs.h"
#include "../module.h"
#include "../lua_helpers.h"

extern "C" {

#include <lua.h>
#include <lauxlib.h>

extern XPLMCreateAvionics_t XPLMCreateAvionics_t_from_table(lua_State* L, int stackpos);
extern void XPLMCreateAvionics_t_to_table(lua_State* L, XPLMCreateAvionics_t const& src);
extern XPLMCreateWindow_t XPLMCreateWindow_t_from_table(lua_State* L, int stackpos);
extern void XPLMCreateWindow_t_to_table(lua_State* L, XPLMCreateWindow_t const& src);
extern XPLMCustomizeAvionics_t XPLMCustomizeAvionics_t_from_table(lua_State* L, int stackpos);
extern void XPLMCustomizeAvionics_t_to_table(lua_State* L, XPLMCustomizeAvionics_t const& src);
extern XPLMFixedString150_t XPLMFixedString150_t_from_table(lua_State* L, int stackpos);
extern void XPLMFixedString150_t_to_table(lua_State* L, XPLMFixedString150_t const& src);

static int cb_XPLMDrawCallback_f(XPLMDrawingPhase inPhase, int inIsBefore, void * inRefcon)
{
	int res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMDrawCallback_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "?br", inPhase, static_cast<bool>(inIsBefore), cb->origRefconRegIndex);
		res = xlua_checkboolean(L, 1) ? 1 : 0;
		lua_pop(L, 1);
	}

	return res;
}

int XLuaRegisterDrawCallback(lua_State* L)
{
	int refcon_regindex = capture_lua_value(L, 4);
	CleanupStoredCallbacks(L, refcon_regindex);

	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, 1, "XPLMDrawCallback_f", refcon_regindex);
	XPLMDrawingPhase inPhase = luaL_checkinteger(L, 2);
	bool inWantsBefore = xlua_checkboolean(L, 3);

	bool res = XPLMRegisterDrawCallback(cb_XPLMDrawCallback_f, inPhase, inWantsBefore, cb_capture_0);
	lua_pushboolean(L, res);

	return 1;
}

int XLuaUnregisterDrawCallback(lua_State* L)
{
	int refcon_regindex = capture_lua_value(L, 4);
	CleanupStoredCallbacks(L, refcon_regindex);

	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, 1, "XPLMDrawCallback_f", refcon_regindex);
	XPLMDrawingPhase inPhase = luaL_checkinteger(L, 2);
	bool inWantsBefore = xlua_checkboolean(L, 3);

	bool res = XPLMUnregisterDrawCallback(cb_XPLMDrawCallback_f, inPhase, inWantsBefore, cb_capture_0);
	lua_pushboolean(L, res);

	return 1;
}

static int cb_XPLMAvionicsCallback_f(XPLMDeviceID inDeviceID, int inIsBefore, void * inRefcon)
{
	int res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMAvionicsCallback_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "?br", inDeviceID, static_cast<bool>(inIsBefore), cb->origRefconRegIndex);
		res = xlua_checkboolean(L, 1) ? 1 : 0;
		lua_pop(L, 1);
	}

	return res;
}

static int cb_XPLMAvionicsMouse_f(int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
{
	int res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMAvionicsMouse_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "ii?r", x, y, inMouse, cb->origRefconRegIndex);
		res = xlua_checkboolean(L, 1) ? 1 : 0;
		lua_pop(L, 1);
	}

	return res;
}

static int cb_XPLMAvionicsMouseWheel_f(int x, int y, int wheel, int clicks, void * inRefcon)
{
	int res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMAvionicsMouseWheel_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "iiiir", x, y, wheel, clicks, cb->origRefconRegIndex);
		res = xlua_checkboolean(L, 1) ? 1 : 0;
		lua_pop(L, 1);
	}

	return res;
}

static XPLMCursorStatus cb_XPLMAvionicsCursor_f(int x, int y, void * inRefcon)
{
	XPLMCursorStatus res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMAvionicsCursor_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "iir", x, y, cb->origRefconRegIndex);
		res = luaL_checkinteger(L, 1);
		lua_pop(L, 1);
	}

	return res;
}

static int cb_XPLMAvionicsKeyboard_f(char inKey, XPLMKeyFlags inFlags, char inVirtualKey, void * inRefCon, int losingFocus)
{
	int res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefCon);
	lua_State* L = setup_lua_callback(cb, "XPLMAvionicsKeyboard_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "s?srb", inKey, inFlags, inVirtualKey, cb->origRefconRegIndex, static_cast<bool>(losingFocus));
		res = xlua_checkboolean(L, 1) ? 1 : 0;
		lua_pop(L, 1);
	}

	return res;
}
/*
 * XPLMCustomizeAvionics_t
 * 
 * Creation and transfer between C struct and Lua table
 *
 */

XPLMCustomizeAvionics_t XPLMCustomizeAvionics_t_from_table(lua_State* L, int stackpos)
{
	XPLMCustomizeAvionics_t out = {};

	luaL_checktype(L, stackpos, LUA_TTABLE);

	lua_getfield(L, -1, "refcon");
	int refcon_0_regindex = capture_lua_value(L, -1);
	lua_pop(L, 1);
	out.structSize = sizeof(out);
	lua_pop(L, 1);

	lua_getfield(L, -1, "deviceId");
	if (lua_isnil(L, -1))
	{
		out.deviceId = static_cast<XPLMDeviceID>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "drawCallbackBefore");
	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, -1, "drawCallbackBefore", refcon_0_regindex);
	out.drawCallbackBefore = [](XPLMDeviceID inDeviceID, int inIsBefore, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "drawCallbackBefore");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "?br", inDeviceID, static_cast<bool>(inIsBefore), cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "drawCallbackAfter");
	wrap_next_lua_func(cb_capture_0, -1, "drawCallbackAfter");
	out.drawCallbackAfter = [](XPLMDeviceID inDeviceID, int inIsBefore, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "drawCallbackAfter");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "?br", inDeviceID, static_cast<bool>(inIsBefore), cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelClickCallback");
	wrap_next_lua_func(cb_capture_0, -1, "bezelClickCallback");
	out.bezelClickCallback = [](int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "bezelClickCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "ii?r", x, y, inMouse, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelRightClickCallback");
	wrap_next_lua_func(cb_capture_0, -1, "bezelRightClickCallback");
	out.bezelRightClickCallback = [](int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "bezelRightClickCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "ii?r", x, y, inMouse, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelScrollCallback");
	wrap_next_lua_func(cb_capture_0, -1, "bezelScrollCallback");
	out.bezelScrollCallback = [](int x, int y, int wheel, int clicks, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "bezelScrollCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "iiiir", x, y, wheel, clicks, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelCursorCallback");
	wrap_next_lua_func(cb_capture_0, -1, "bezelCursorCallback");
	out.bezelCursorCallback = [](int x, int y, void * inRefcon)
		{
			XPLMCursorStatus res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "bezelCursorCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "iir", x, y, cb->origRefconRegIndex);
				res = luaL_checkinteger(L, 1);
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenTouchCallback");
	wrap_next_lua_func(cb_capture_0, -1, "screenTouchCallback");
	out.screenTouchCallback = [](int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "screenTouchCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "ii?r", x, y, inMouse, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenRightTouchCallback");
	wrap_next_lua_func(cb_capture_0, -1, "screenRightTouchCallback");
	out.screenRightTouchCallback = [](int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "screenRightTouchCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "ii?r", x, y, inMouse, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenScrollCallback");
	wrap_next_lua_func(cb_capture_0, -1, "screenScrollCallback");
	out.screenScrollCallback = [](int x, int y, int wheel, int clicks, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "screenScrollCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "iiiir", x, y, wheel, clicks, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenCursorCallback");
	wrap_next_lua_func(cb_capture_0, -1, "screenCursorCallback");
	out.screenCursorCallback = [](int x, int y, void * inRefcon)
		{
			XPLMCursorStatus res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "screenCursorCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "iir", x, y, cb->origRefconRegIndex);
				res = luaL_checkinteger(L, 1);
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "keyboardCallback");
	wrap_next_lua_func(cb_capture_0, -1, "XPLMAvionicsKeyboard_f");
	lua_pop(L, 1);

	lua_getfield(L, -1, "refcon");
	if (lua_isnil(L, -1))
	{
		out.refcon = static_cast<void *>(xlua_checkuserdata<void*>(L, -1, "Expected userdata<void*>"));
	}
	lua_pop(L, 1);

	return out;
}

void XPLMCustomizeAvionics_t_to_table(lua_State* L, XPLMCustomizeAvionics_t const& src)
{
	lua_newtable(L);

	lua_pushstring(L, "deviceId");
	lua_pushinteger(L, src.deviceId);
	lua_settable(L, -3);

	lua_pushstring(L, "refcon");
	xlua_pushuserdata<void*>(L, src.refcon);
	lua_settable(L, -3);
}

int MakeXPLMCustomizeAvionics_t(lua_State* L)
{
	XPLMCustomizeAvionics_t out = {};
	out.structSize = sizeof(XPLMCustomizeAvionics_t);
	XPLMCustomizeAvionics_t_to_table(L, out);
	return 1;
}
/*
 * END Creation and transfer between C struct and Lua table
 *
 */

int XLuaRegisterAvionicsCallbacksEx(lua_State* L)
{
	XPLMCustomizeAvionics_t inParams = XPLMCustomizeAvionics_t_from_table(L, 1);

	XPLMAvionicsID res = XPLMRegisterAvionicsCallbacksEx(&inParams);
	if (res == nullptr)
	{
		lua_pushnil(L);
	}
	else
	{
		xlua_pushuserdata<XPLMAvionicsID>(L, res);
	}

	return 1;
}

int XLuaGetAvionicsHandle(lua_State* L)
{
	XPLMDeviceID inDeviceID = luaL_checkinteger(L, 1);

	XPLMAvionicsID res = XPLMGetAvionicsHandle(inDeviceID);
	if (res == nullptr)
	{
		lua_pushnil(L);
	}
	else
	{
		xlua_pushuserdata<XPLMAvionicsID>(L, res);
	}

	return 1;
}

int XLuaUnregisterAvionicsCallbacks(lua_State* L)
{
	XPLMAvionicsID inAvionicsId = {};
	if (lua_isuserdata(L, 1))
	{
		inAvionicsId = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	XPLMUnregisterAvionicsCallbacks(inAvionicsId);

	return 0;
}

static void cb_XPLMAvionicsScreenCallback_f(void * inRefcon)
{
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMAvionicsScreenCallback_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), false, "r", cb->origRefconRegIndex);
	}
}

static void cb_XPLMAvionicsBezelCallback_f(float inAmbiantR, float inAmbiantG, float inAmbiantB, void * inRefcon)
{
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMAvionicsBezelCallback_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), false, "fffr", inAmbiantR, inAmbiantG, inAmbiantB, cb->origRefconRegIndex);
	}
}

static float cb_XPLMAvionicsBrightness_f(float inRheoValue, float inAmbiantBrightness, float inBusVoltsRatio, void * inRefcon)
{
	float res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMAvionicsBrightness_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "fffr", inRheoValue, inAmbiantBrightness, inBusVoltsRatio, cb->origRefconRegIndex);
		res = luaL_checknumber(L, 1);
		lua_pop(L, 1);
	}

	return res;
}
/*
 * XPLMCreateAvionics_t
 * 
 * Creation and transfer between C struct and Lua table
 *
 */

XPLMCreateAvionics_t XPLMCreateAvionics_t_from_table(lua_State* L, int stackpos)
{
	XPLMCreateAvionics_t out = {};

	luaL_checktype(L, stackpos, LUA_TTABLE);

	lua_getfield(L, -1, "refcon");
	int refcon_0_regindex = capture_lua_value(L, -1);
	lua_pop(L, 1);
	out.structSize = sizeof(out);
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenWidth");
	if (lua_isnil(L, -1))
	{
		out.screenWidth = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenHeight");
	if (lua_isnil(L, -1))
	{
		out.screenHeight = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelWidth");
	if (lua_isnil(L, -1))
	{
		out.bezelWidth = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelHeight");
	if (lua_isnil(L, -1))
	{
		out.bezelHeight = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenOffsetX");
	if (lua_isnil(L, -1))
	{
		out.screenOffsetX = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenOffsetY");
	if (lua_isnil(L, -1))
	{
		out.screenOffsetY = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "drawOnDemand");
	if (lua_isnil(L, -1))
	{
		out.drawOnDemand = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelDrawCallback");
	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, -1, "XPLMAvionicsBezelCallback_f", refcon_0_regindex);
	lua_pop(L, 1);

	lua_getfield(L, -1, "drawCallback");
	wrap_next_lua_func(cb_capture_0, -1, "XPLMAvionicsScreenCallback_f");
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelClickCallback");
	wrap_next_lua_func(cb_capture_0, -1, "bezelClickCallback");
	out.bezelClickCallback = [](int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "bezelClickCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "ii?r", x, y, inMouse, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelRightClickCallback");
	wrap_next_lua_func(cb_capture_0, -1, "bezelRightClickCallback");
	out.bezelRightClickCallback = [](int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "bezelRightClickCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "ii?r", x, y, inMouse, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelScrollCallback");
	wrap_next_lua_func(cb_capture_0, -1, "bezelScrollCallback");
	out.bezelScrollCallback = [](int x, int y, int wheel, int clicks, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "bezelScrollCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "iiiir", x, y, wheel, clicks, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "bezelCursorCallback");
	wrap_next_lua_func(cb_capture_0, -1, "bezelCursorCallback");
	out.bezelCursorCallback = [](int x, int y, void * inRefcon)
		{
			XPLMCursorStatus res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "bezelCursorCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "iir", x, y, cb->origRefconRegIndex);
				res = luaL_checkinteger(L, 1);
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenTouchCallback");
	wrap_next_lua_func(cb_capture_0, -1, "screenTouchCallback");
	out.screenTouchCallback = [](int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "screenTouchCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "ii?r", x, y, inMouse, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenRightTouchCallback");
	wrap_next_lua_func(cb_capture_0, -1, "screenRightTouchCallback");
	out.screenRightTouchCallback = [](int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "screenRightTouchCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "ii?r", x, y, inMouse, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenScrollCallback");
	wrap_next_lua_func(cb_capture_0, -1, "screenScrollCallback");
	out.screenScrollCallback = [](int x, int y, int wheel, int clicks, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "screenScrollCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "iiiir", x, y, wheel, clicks, cb->origRefconRegIndex);
				res = xlua_checkboolean(L, 1) ? 1 : 0;
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "screenCursorCallback");
	wrap_next_lua_func(cb_capture_0, -1, "screenCursorCallback");
	out.screenCursorCallback = [](int x, int y, void * inRefcon)
		{
			XPLMCursorStatus res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "screenCursorCallback");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "iir", x, y, cb->origRefconRegIndex);
				res = luaL_checkinteger(L, 1);
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "keyboardCallback");
	wrap_next_lua_func(cb_capture_0, -1, "XPLMAvionicsKeyboard_f");
	lua_pop(L, 1);

	lua_getfield(L, -1, "brightnessCallback");
	wrap_next_lua_func(cb_capture_0, -1, "XPLMAvionicsBrightness_f");
	lua_pop(L, 1);

	lua_getfield(L, -1, "deviceID");
	if (lua_isnil(L, -1))
	{
		out.deviceID = static_cast<char const*>(luaL_checkstring(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "deviceName");
	if (lua_isnil(L, -1))
	{
		out.deviceName = static_cast<char const *>(luaL_checkstring(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "refcon");
	if (lua_isnil(L, -1))
	{
		out.refcon = static_cast<void *>(xlua_checkuserdata<void*>(L, -1, "Expected userdata<void*>"));
	}
	lua_pop(L, 1);

	return out;
}

void XPLMCreateAvionics_t_to_table(lua_State* L, XPLMCreateAvionics_t const& src)
{
	lua_newtable(L);

	lua_pushstring(L, "screenWidth");
	lua_pushinteger(L, src.screenWidth);
	lua_settable(L, -3);

	lua_pushstring(L, "screenHeight");
	lua_pushinteger(L, src.screenHeight);
	lua_settable(L, -3);

	lua_pushstring(L, "bezelWidth");
	lua_pushinteger(L, src.bezelWidth);
	lua_settable(L, -3);

	lua_pushstring(L, "bezelHeight");
	lua_pushinteger(L, src.bezelHeight);
	lua_settable(L, -3);

	lua_pushstring(L, "screenOffsetX");
	lua_pushinteger(L, src.screenOffsetX);
	lua_settable(L, -3);

	lua_pushstring(L, "screenOffsetY");
	lua_pushinteger(L, src.screenOffsetY);
	lua_settable(L, -3);

	lua_pushstring(L, "drawOnDemand");
	lua_pushboolean(L, src.drawOnDemand);
	lua_settable(L, -3);

	lua_pushstring(L, "bezelDrawCallback");
	luaL_loadstring(L, "function() end");
	lua_settable(L, -3);

	lua_pushstring(L, "drawCallback");
	luaL_loadstring(L, "function() end");
	lua_settable(L, -3);

	lua_pushstring(L, "deviceID");
	lua_pushstring(L, src.deviceID);
	lua_settable(L, -3);

	lua_pushstring(L, "deviceName");
	lua_pushstring(L, src.deviceName);
	lua_settable(L, -3);

	lua_pushstring(L, "refcon");
	xlua_pushuserdata<void*>(L, src.refcon);
	lua_settable(L, -3);
}

int MakeXPLMCreateAvionics_t(lua_State* L)
{
	XPLMCreateAvionics_t out = {};
	out.structSize = sizeof(XPLMCreateAvionics_t);
	XPLMCreateAvionics_t_to_table(L, out);
	return 1;
}
/*
 * END Creation and transfer between C struct and Lua table
 *
 */

int XLuaCreateAvionicsEx(lua_State* L)
{
	XPLMCreateAvionics_t inParams = XPLMCreateAvionics_t_from_table(L, 1);

	XPLMAvionicsID res = XPLMCreateAvionicsEx(&inParams);
	if (res == nullptr)
	{
		lua_pushnil(L);
	}
	else
	{
		xlua_pushuserdata<XPLMAvionicsID>(L, res);
	}

	return 1;
}

int XLuaDestroyAvionics(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	XPLMDestroyAvionics(inHandle);

	return 0;
}

int XLuaIsAvionicsBound(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	bool res = XPLMIsAvionicsBound(inHandle);
	lua_pushboolean(L, res);

	return 1;
}

int XLuaSetAvionicsBrightnessRheo(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}
	float brightness = luaL_checknumber(L, 2);

	XPLMSetAvionicsBrightnessRheo(inHandle, brightness);

	return 0;
}

int XLuaGetAvionicsBrightnessRheo(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	float res = XPLMGetAvionicsBrightnessRheo(inHandle);
	lua_pushnumber(L, res);

	return 1;
}

int XLuaGetAvionicsBusVoltsRatio(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	float res = XPLMGetAvionicsBusVoltsRatio(inHandle);
	lua_pushnumber(L, res);

	return 1;
}

int XLuaIsCursorOverAvionics(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}
	int outX;
	int outY;

	bool res = XPLMIsCursorOverAvionics(inHandle, &outX, &outY);
	lua_pushboolean(L, res);

	lua_createtable(L, 0, 2); // 0 array slots and 2 key-value pairs

	lua_pushstring(L, "outX");
	lua_pushinteger(L, outX);
	lua_settable(L, -3);

	lua_pushstring(L, "outY");
	lua_pushinteger(L, outY);
	lua_settable(L, -3);

	return 2;
}

int XLuaAvionicsNeedsDrawing(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	XPLMAvionicsNeedsDrawing(inHandle);

	return 0;
}

int XLuaSetAvionicsPopupVisible(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}
	bool inVisible = xlua_checkboolean(L, 2);

	XPLMSetAvionicsPopupVisible(inHandle, inVisible);

	return 0;
}

int XLuaIsAvionicsPopupVisible(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	bool res = XPLMIsAvionicsPopupVisible(inHandle);
	lua_pushboolean(L, res);

	return 1;
}

int XLuaPopOutAvionics(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	XPLMPopOutAvionics(inHandle);

	return 0;
}

int XLuaIsAvionicsPoppedOut(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	bool res = XPLMIsAvionicsPoppedOut(inHandle);
	lua_pushboolean(L, res);

	return 1;
}

int XLuaTakeAvionicsKeyboardFocus(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	XPLMTakeAvionicsKeyboardFocus(inHandle);

	return 0;
}

int XLuaHasAvionicsKeyboardFocus(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}

	bool res = XPLMHasAvionicsKeyboardFocus(inHandle);
	lua_pushboolean(L, res);

	return 1;
}

int XLuaGetAvionicsGeometry(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}
	int outLeft;
	int outTop;
	int outRight;
	int outBottom;

	XPLMGetAvionicsGeometry(inHandle, &outLeft, &outTop, &outRight, &outBottom);

	lua_createtable(L, 0, 4); // 0 array slots and 4 key-value pairs

	lua_pushstring(L, "outLeft");
	lua_pushinteger(L, outLeft);
	lua_settable(L, -3);

	lua_pushstring(L, "outTop");
	lua_pushinteger(L, outTop);
	lua_settable(L, -3);

	lua_pushstring(L, "outRight");
	lua_pushinteger(L, outRight);
	lua_settable(L, -3);

	lua_pushstring(L, "outBottom");
	lua_pushinteger(L, outBottom);
	lua_settable(L, -3);

	return 1;
}

int XLuaSetAvionicsGeometry(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}
	int inLeft = luaL_checkinteger(L, 2);
	int inTop = luaL_checkinteger(L, 3);
	int inRight = luaL_checkinteger(L, 4);
	int inBottom = luaL_checkinteger(L, 5);

	XPLMSetAvionicsGeometry(inHandle, inLeft, inTop, inRight, inBottom);

	return 0;
}

int XLuaGetAvionicsGeometryOS(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}
	int outLeft;
	int outTop;
	int outRight;
	int outBottom;

	XPLMGetAvionicsGeometryOS(inHandle, &outLeft, &outTop, &outRight, &outBottom);

	lua_createtable(L, 0, 4); // 0 array slots and 4 key-value pairs

	lua_pushstring(L, "outLeft");
	lua_pushinteger(L, outLeft);
	lua_settable(L, -3);

	lua_pushstring(L, "outTop");
	lua_pushinteger(L, outTop);
	lua_settable(L, -3);

	lua_pushstring(L, "outRight");
	lua_pushinteger(L, outRight);
	lua_settable(L, -3);

	lua_pushstring(L, "outBottom");
	lua_pushinteger(L, outBottom);
	lua_settable(L, -3);

	return 1;
}

int XLuaSetAvionicsGeometryOS(lua_State* L)
{
	XPLMAvionicsID inHandle = {};
	if (lua_isuserdata(L, 1))
	{
		inHandle = xlua_checkuserdata<XPLMAvionicsID>(L, 1, "Expected userdata<XPLMAvionicsID>");
	}
	int inLeft = luaL_checkinteger(L, 2);
	int inTop = luaL_checkinteger(L, 3);
	int inRight = luaL_checkinteger(L, 4);
	int inBottom = luaL_checkinteger(L, 5);

	XPLMSetAvionicsGeometryOS(inHandle, inLeft, inTop, inRight, inBottom);

	return 0;
}

static void cb_XPLMDrawWindow_f(XPLMWindowID inWindowID, void * inRefcon)
{
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMDrawWindow_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), false, "?r", inWindowID, cb->origRefconRegIndex);
	}
}

static void cb_XPLMHandleKey_f(XPLMWindowID inWindowID, char inKey, XPLMKeyFlags inFlags, char inVirtualKey, void * inRefcon, int losingFocus)
{
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMHandleKey_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), false, "?s?srb", inWindowID, inKey, inFlags, inVirtualKey, cb->origRefconRegIndex, static_cast<bool>(losingFocus));
	}
}

static int cb_XPLMHandleMouseClick_f(XPLMWindowID inWindowID, int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
{
	int res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMHandleMouseClick_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "?ii?r", inWindowID, x, y, inMouse, cb->origRefconRegIndex);
		res = luaL_checkinteger(L, 1);
		lua_pop(L, 1);
	}

	return res;
}

static XPLMCursorStatus cb_XPLMHandleCursor_f(XPLMWindowID inWindowID, int x, int y, void * inRefcon)
{
	XPLMCursorStatus res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMHandleCursor_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "?iir", inWindowID, x, y, cb->origRefconRegIndex);
		res = luaL_checkinteger(L, 1);
		lua_pop(L, 1);
	}

	return res;
}

static int cb_XPLMHandleMouseWheel_f(XPLMWindowID inWindowID, int x, int y, int wheel, int clicks, void * inRefcon)
{
	int res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMHandleMouseWheel_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "?iiiir", inWindowID, x, y, wheel, clicks, cb->origRefconRegIndex);
		res = xlua_checkboolean(L, 1) ? 1 : 0;
		lua_pop(L, 1);
	}

	return res;
}
/*
 * XPLMCreateWindow_t
 * 
 * Creation and transfer between C struct and Lua table
 *
 */

XPLMCreateWindow_t XPLMCreateWindow_t_from_table(lua_State* L, int stackpos)
{
	XPLMCreateWindow_t out = {};

	luaL_checktype(L, stackpos, LUA_TTABLE);

	lua_getfield(L, -1, "refcon");
	int refcon_0_regindex = capture_lua_value(L, -1);
	lua_pop(L, 1);
	out.structSize = sizeof(out);
	lua_pop(L, 1);

	lua_getfield(L, -1, "left");
	if (lua_isnil(L, -1))
	{
		out.left = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "top");
	if (lua_isnil(L, -1))
	{
		out.top = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "right");
	if (lua_isnil(L, -1))
	{
		out.right = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "bottom");
	if (lua_isnil(L, -1))
	{
		out.bottom = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "visible");
	if (lua_isnil(L, -1))
	{
		out.visible = static_cast<int>(luaL_checkinteger(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "drawWindowFunc");
	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, -1, "XPLMDrawWindow_f", refcon_0_regindex);
	lua_pop(L, 1);

	lua_getfield(L, -1, "handleMouseClickFunc");
	wrap_next_lua_func(cb_capture_0, -1, "handleMouseClickFunc");
	out.handleMouseClickFunc = [](XPLMWindowID inWindowID, int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "handleMouseClickFunc");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "?ii?r", inWindowID, x, y, inMouse, cb->origRefconRegIndex);
				res = luaL_checkinteger(L, 1);
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	lua_getfield(L, -1, "handleKeyFunc");
	wrap_next_lua_func(cb_capture_0, -1, "XPLMHandleKey_f");
	lua_pop(L, 1);

	lua_getfield(L, -1, "handleCursorFunc");
	wrap_next_lua_func(cb_capture_0, -1, "XPLMHandleCursor_f");
	lua_pop(L, 1);

	lua_getfield(L, -1, "handleMouseWheelFunc");
	wrap_next_lua_func(cb_capture_0, -1, "XPLMHandleMouseWheel_f");
	lua_pop(L, 1);

	lua_getfield(L, -1, "refcon");
	if (lua_isnil(L, -1))
	{
		out.refcon = static_cast<void *>(xlua_checkuserdata<void*>(L, -1, "Expected userdata<void*>"));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "decorateAsFloatingWindow");
	if (lua_isnil(L, -1))
	{
		out.decorateAsFloatingWindow = static_cast<XPLMWindowDecoration>(xlua_checkboolean(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "layer");
	if (lua_isnil(L, -1))
	{
		out.layer = static_cast<XPLMWindowLayer>(xlua_checkboolean(L, -1));
	}
	lua_pop(L, 1);

	lua_getfield(L, -1, "handleRightClickFunc");
	wrap_next_lua_func(cb_capture_0, -1, "handleRightClickFunc");
	out.handleRightClickFunc = [](XPLMWindowID inWindowID, int x, int y, XPLMMouseStatus inMouse, void * inRefcon)
		{
			int res = {};
			notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
			lua_State* L = setup_lua_callback(cb, "handleRightClickFunc");
			if (L)
			{
				fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "?ii?r", inWindowID, x, y, inMouse, cb->origRefconRegIndex);
				res = luaL_checkinteger(L, 1);
				lua_pop(L, 1);
			}

			return res;
		};
	lua_pop(L, 1);

	return out;
}

void XPLMCreateWindow_t_to_table(lua_State* L, XPLMCreateWindow_t const& src)
{
	lua_newtable(L);

	lua_pushstring(L, "left");
	lua_pushinteger(L, src.left);
	lua_settable(L, -3);

	lua_pushstring(L, "top");
	lua_pushinteger(L, src.top);
	lua_settable(L, -3);

	lua_pushstring(L, "right");
	lua_pushinteger(L, src.right);
	lua_settable(L, -3);

	lua_pushstring(L, "bottom");
	lua_pushinteger(L, src.bottom);
	lua_settable(L, -3);

	lua_pushstring(L, "visible");
	lua_pushboolean(L, src.visible);
	lua_settable(L, -3);

	lua_pushstring(L, "drawWindowFunc");
	luaL_loadstring(L, "function() end");
	lua_settable(L, -3);

	lua_pushstring(L, "handleKeyFunc");
	luaL_loadstring(L, "function() end");
	lua_settable(L, -3);

	lua_pushstring(L, "refcon");
	xlua_pushuserdata<void*>(L, src.refcon);
	lua_settable(L, -3);

	lua_pushstring(L, "decorateAsFloatingWindow");
	lua_pushboolean(L, src.decorateAsFloatingWindow);
	lua_settable(L, -3);

	lua_pushstring(L, "layer");
	lua_pushboolean(L, src.layer);
	lua_settable(L, -3);
}

int MakeXPLMCreateWindow_t(lua_State* L)
{
	XPLMCreateWindow_t out = {};
	out.structSize = sizeof(XPLMCreateWindow_t);
	XPLMCreateWindow_t_to_table(L, out);
	return 1;
}
/*
 * END Creation and transfer between C struct and Lua table
 *
 */

int XLuaCreateWindowEx(lua_State* L)
{
	XPLMCreateWindow_t inParams = XPLMCreateWindow_t_from_table(L, 1);

	XPLMWindowID res = XPLMCreateWindowEx(&inParams);
	if (res == nullptr)
	{
		lua_pushnil(L);
	}
	else
	{
		xlua_pushuserdata<XPLMWindowID>(L, res);
	}

	return 1;
}

int XLuaCreateWindow(lua_State* L)
{
	int inLeft = luaL_checkinteger(L, 1);
	int inTop = luaL_checkinteger(L, 2);
	int inRight = luaL_checkinteger(L, 3);
	int inBottom = luaL_checkinteger(L, 4);
	bool inIsVisible = xlua_checkboolean(L, 5);
	int refcon_regindex = capture_lua_value(L, 9);
	CleanupStoredCallbacks(L, refcon_regindex);

	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, 6, "XPLMDrawWindow_f", refcon_regindex);
	wrap_next_lua_func(cb_capture_0, 7, "XPLMHandleKey_f");
	wrap_next_lua_func(cb_capture_0, 8, "XPLMHandleMouseClick_f");

	XPLMWindowID res = XPLMCreateWindow(inLeft, inTop, inRight, inBottom, inIsVisible, cb_XPLMDrawWindow_f, cb_XPLMHandleKey_f, cb_XPLMHandleMouseClick_f, cb_capture_0);
	if (res == nullptr)
	{
		lua_pushnil(L);
	}
	else
	{
		xlua_pushuserdata<XPLMWindowID>(L, res);
	}

	return 1;
}

int XLuaDestroyWindow(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}

	XPLMDestroyWindow(inWindowID);

	return 0;
}

int XLuaGetScreenSize(lua_State* L)
{
	int outWidth;
	int outHeight;
	XPLMGetScreenSize(&outWidth, &outHeight);

	lua_createtable(L, 0, 2); // 0 array slots and 2 key-value pairs

	lua_pushstring(L, "outWidth");
	lua_pushinteger(L, outWidth);
	lua_settable(L, -3);

	lua_pushstring(L, "outHeight");
	lua_pushinteger(L, outHeight);
	lua_settable(L, -3);

	return 1;
}

int XLuaGetScreenBoundsGlobal(lua_State* L)
{
	int outLeft;
	int outTop;
	int outRight;
	int outBottom;
	XPLMGetScreenBoundsGlobal(&outLeft, &outTop, &outRight, &outBottom);

	lua_createtable(L, 0, 4); // 0 array slots and 4 key-value pairs

	lua_pushstring(L, "outLeft");
	lua_pushinteger(L, outLeft);
	lua_settable(L, -3);

	lua_pushstring(L, "outTop");
	lua_pushinteger(L, outTop);
	lua_settable(L, -3);

	lua_pushstring(L, "outRight");
	lua_pushinteger(L, outRight);
	lua_settable(L, -3);

	lua_pushstring(L, "outBottom");
	lua_pushinteger(L, outBottom);
	lua_settable(L, -3);

	return 1;
}

static void cb_XPLMReceiveMonitorBoundsGlobal_f(int inMonitorIndex, int inLeftBx, int inTopBx, int inRightBx, int inBottomBx, void * inRefcon)
{
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMReceiveMonitorBoundsGlobal_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), false, "iiiiir", inMonitorIndex, inLeftBx, inTopBx, inRightBx, inBottomBx, cb->origRefconRegIndex);
	}
}

int XLuaGetAllMonitorBoundsGlobal(lua_State* L)
{
	int refcon_regindex = capture_lua_value(L, 2);
	CleanupStoredCallbacks(L, refcon_regindex);

	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, 1, "XPLMReceiveMonitorBoundsGlobal_f", refcon_regindex);

	XPLMGetAllMonitorBoundsGlobal(cb_XPLMReceiveMonitorBoundsGlobal_f, cb_capture_0);

	return 0;
}

static void cb_XPLMReceiveMonitorBoundsOS_f(int inMonitorIndex, int inLeftPx, int inTopPx, int inRightPx, int inBottomPx, void * inRefcon)
{
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMReceiveMonitorBoundsOS_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), false, "iiiiir", inMonitorIndex, inLeftPx, inTopPx, inRightPx, inBottomPx, cb->origRefconRegIndex);
	}
}

int XLuaGetAllMonitorBoundsOS(lua_State* L)
{
	int refcon_regindex = capture_lua_value(L, 2);
	CleanupStoredCallbacks(L, refcon_regindex);

	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, 1, "XPLMReceiveMonitorBoundsOS_f", refcon_regindex);

	XPLMGetAllMonitorBoundsOS(cb_XPLMReceiveMonitorBoundsOS_f, cb_capture_0);

	return 0;
}

int XLuaGetMouseLocation(lua_State* L)
{
	int outX;
	int outY;
	XPLMGetMouseLocation(&outX, &outY);

	lua_createtable(L, 0, 2); // 0 array slots and 2 key-value pairs

	lua_pushstring(L, "outX");
	lua_pushinteger(L, outX);
	lua_settable(L, -3);

	lua_pushstring(L, "outY");
	lua_pushinteger(L, outY);
	lua_settable(L, -3);

	return 1;
}

int XLuaGetMouseLocationGlobal(lua_State* L)
{
	int outX;
	int outY;
	XPLMGetMouseLocationGlobal(&outX, &outY);

	lua_createtable(L, 0, 2); // 0 array slots and 2 key-value pairs

	lua_pushstring(L, "outX");
	lua_pushinteger(L, outX);
	lua_settable(L, -3);

	lua_pushstring(L, "outY");
	lua_pushinteger(L, outY);
	lua_settable(L, -3);

	return 1;
}

int XLuaGetWindowGeometry(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	int outLeft;
	int outTop;
	int outRight;
	int outBottom;

	XPLMGetWindowGeometry(inWindowID, &outLeft, &outTop, &outRight, &outBottom);

	lua_createtable(L, 0, 4); // 0 array slots and 4 key-value pairs

	lua_pushstring(L, "outLeft");
	lua_pushinteger(L, outLeft);
	lua_settable(L, -3);

	lua_pushstring(L, "outTop");
	lua_pushinteger(L, outTop);
	lua_settable(L, -3);

	lua_pushstring(L, "outRight");
	lua_pushinteger(L, outRight);
	lua_settable(L, -3);

	lua_pushstring(L, "outBottom");
	lua_pushinteger(L, outBottom);
	lua_settable(L, -3);

	return 1;
}

int XLuaSetWindowGeometry(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	int inLeft = luaL_checkinteger(L, 2);
	int inTop = luaL_checkinteger(L, 3);
	int inRight = luaL_checkinteger(L, 4);
	int inBottom = luaL_checkinteger(L, 5);

	XPLMSetWindowGeometry(inWindowID, inLeft, inTop, inRight, inBottom);

	return 0;
}

int XLuaGetWindowGeometryOS(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	int outLeft;
	int outTop;
	int outRight;
	int outBottom;

	XPLMGetWindowGeometryOS(inWindowID, &outLeft, &outTop, &outRight, &outBottom);

	lua_createtable(L, 0, 4); // 0 array slots and 4 key-value pairs

	lua_pushstring(L, "outLeft");
	lua_pushinteger(L, outLeft);
	lua_settable(L, -3);

	lua_pushstring(L, "outTop");
	lua_pushinteger(L, outTop);
	lua_settable(L, -3);

	lua_pushstring(L, "outRight");
	lua_pushinteger(L, outRight);
	lua_settable(L, -3);

	lua_pushstring(L, "outBottom");
	lua_pushinteger(L, outBottom);
	lua_settable(L, -3);

	return 1;
}

int XLuaSetWindowGeometryOS(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	int inLeft = luaL_checkinteger(L, 2);
	int inTop = luaL_checkinteger(L, 3);
	int inRight = luaL_checkinteger(L, 4);
	int inBottom = luaL_checkinteger(L, 5);

	XPLMSetWindowGeometryOS(inWindowID, inLeft, inTop, inRight, inBottom);

	return 0;
}

int XLuaGetWindowGeometryVR(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	int outWidthBoxels;
	int outHeightBoxels;

	XPLMGetWindowGeometryVR(inWindowID, &outWidthBoxels, &outHeightBoxels);

	lua_createtable(L, 0, 2); // 0 array slots and 2 key-value pairs

	lua_pushstring(L, "outWidthBoxels");
	lua_pushinteger(L, outWidthBoxels);
	lua_settable(L, -3);

	lua_pushstring(L, "outHeightBoxels");
	lua_pushinteger(L, outHeightBoxels);
	lua_settable(L, -3);

	return 1;
}

int XLuaSetWindowGeometryVR(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	int widthBoxels = luaL_checkinteger(L, 2);
	int heightBoxels = luaL_checkinteger(L, 3);

	XPLMSetWindowGeometryVR(inWindowID, widthBoxels, heightBoxels);

	return 0;
}

int XLuaGetWindowIsVisible(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}

	bool res = XPLMGetWindowIsVisible(inWindowID);
	lua_pushboolean(L, res);

	return 1;
}

int XLuaSetWindowIsVisible(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	bool inIsVisible = xlua_checkboolean(L, 2);

	XPLMSetWindowIsVisible(inWindowID, inIsVisible);

	return 0;
}

int XLuaWindowIsPoppedOut(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}

	bool res = XPLMWindowIsPoppedOut(inWindowID);
	lua_pushboolean(L, res);

	return 1;
}

int XLuaWindowIsInVR(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}

	bool res = XPLMWindowIsInVR(inWindowID);
	lua_pushboolean(L, res);

	return 1;
}

int XLuaSetWindowGravity(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	float inLeftGravity = luaL_checknumber(L, 2);
	float inTopGravity = luaL_checknumber(L, 3);
	float inRightGravity = luaL_checknumber(L, 4);
	float inBottomGravity = luaL_checknumber(L, 5);

	XPLMSetWindowGravity(inWindowID, inLeftGravity, inTopGravity, inRightGravity, inBottomGravity);

	return 0;
}

int XLuaSetWindowResizingLimits(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	int inMinWidthBoxels = luaL_checkinteger(L, 2);
	int inMinHeightBoxels = luaL_checkinteger(L, 3);
	int inMaxWidthBoxels = luaL_checkinteger(L, 4);
	int inMaxHeightBoxels = luaL_checkinteger(L, 5);

	XPLMSetWindowResizingLimits(inWindowID, inMinWidthBoxels, inMinHeightBoxels, inMaxWidthBoxels, inMaxHeightBoxels);

	return 0;
}

int XLuaSetWindowPositioningMode(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	XPLMWindowPositioningMode inPositioningMode = luaL_checkinteger(L, 2);
	int inMonitorIndex = luaL_checkinteger(L, 3);

	XPLMSetWindowPositioningMode(inWindowID, inPositioningMode, inMonitorIndex);

	return 0;
}

int XLuaSetWindowTitle(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	const char * inWindowTitle = luaL_checkstring(L, 2);

	XPLMSetWindowTitle(inWindowID, inWindowTitle);

	return 0;
}

int XLuaGetWindowRefCon(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}

	void * res = XPLMGetWindowRefCon(inWindowID);
	xlua_pushuserdata<void*>(L, res);

	return 1;
}

int XLuaSetWindowRefCon(lua_State* L)
{
	XPLMWindowID inWindowID = {};
	if (lua_isuserdata(L, 1))
	{
		inWindowID = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}
	void * inRefcon = {};
	if (lua_isuserdata(L, 2))
	{
		inRefcon = xlua_checkuserdata<void*>(L, 2, "Expected userdata<void*>");
	}

	XPLMSetWindowRefCon(inWindowID, inRefcon);

	return 0;
}

int XLuaTakeKeyboardFocus(lua_State* L)
{
	XPLMWindowID inWindow = {};
	if (lua_isuserdata(L, 1))
	{
		inWindow = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}

	XPLMTakeKeyboardFocus(inWindow);

	return 0;
}

int XLuaHasKeyboardFocus(lua_State* L)
{
	XPLMWindowID inWindow = {};
	if (lua_isuserdata(L, 1))
	{
		inWindow = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}

	bool res = XPLMHasKeyboardFocus(inWindow);
	lua_pushboolean(L, res);

	return 1;
}

int XLuaBringWindowToFront(lua_State* L)
{
	XPLMWindowID inWindow = {};
	if (lua_isuserdata(L, 1))
	{
		inWindow = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}

	XPLMBringWindowToFront(inWindow);

	return 0;
}

int XLuaIsWindowInFront(lua_State* L)
{
	XPLMWindowID inWindow = {};
	if (lua_isuserdata(L, 1))
	{
		inWindow = xlua_checkuserdata<XPLMWindowID>(L, 1, "Expected userdata<XPLMWindowID>");
	}

	bool res = XPLMIsWindowInFront(inWindow);
	lua_pushboolean(L, res);

	return 1;
}

static int cb_XPLMKeySniffer_f(char inChar, XPLMKeyFlags inFlags, char inVirtualKey, void * inRefcon)
{
	int res = {};
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMKeySniffer_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), true, "s?sr", inChar, inFlags, inVirtualKey, cb->origRefconRegIndex);
		res = xlua_checkboolean(L, 1) ? 1 : 0;
		lua_pop(L, 1);
	}

	return res;
}

int XLuaRegisterKeySniffer(lua_State* L)
{
	int refcon_regindex = capture_lua_value(L, 3);
	CleanupStoredCallbacks(L, refcon_regindex);

	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, 1, "XPLMKeySniffer_f", refcon_regindex);
	bool inBeforeWindows = xlua_checkboolean(L, 2);

	bool res = XPLMRegisterKeySniffer(cb_XPLMKeySniffer_f, inBeforeWindows, cb_capture_0);
	lua_pushboolean(L, res);

	return 1;
}

int XLuaUnregisterKeySniffer(lua_State* L)
{
	int refcon_regindex = capture_lua_value(L, 3);
	CleanupStoredCallbacks(L, refcon_regindex);

	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, 1, "XPLMKeySniffer_f", refcon_regindex);
	bool inBeforeWindows = xlua_checkboolean(L, 2);

	bool res = XPLMUnregisterKeySniffer(cb_XPLMKeySniffer_f, inBeforeWindows, cb_capture_0);
	lua_pushboolean(L, res);

	return 1;
}

static void cb_XPLMHotKey_f(void * inRefcon)
{
	notify_cb_t* cb = static_cast<notify_cb_t*>(inRefcon);
	lua_State* L = setup_lua_callback(cb, "XPLMHotKey_f");
	if (L)
	{
		fmt_pcall_stdvars(L, module::debug_proc_from_interp(L), false, "r", cb->origRefconRegIndex);
	}
}

int XLuaRegisterHotKey(lua_State* L)
{
	std::string inVirtualKey_s = luaL_checkstring(L, 1);
	if (inVirtualKey_s.size() != 1)
	{
		luaL_argerror(L, 1, "inVirtualKey must be exactly one character");
		return 0;
	}
	char inVirtualKey = inVirtualKey_s[0];
	XPLMKeyFlags inFlags = luaL_checkinteger(L, 2);
	const char * inDescription = luaL_checkstring(L, 3);
	int refcon_regindex = capture_lua_value(L, 5);
	CleanupStoredCallbacks(L, refcon_regindex);

	notify_cb_t* cb_capture_0 = wrap_first_lua_func(L, 4, "XPLMHotKey_f", refcon_regindex);

	XPLMHotKeyID res = XPLMRegisterHotKey(inVirtualKey, inFlags, inDescription, cb_XPLMHotKey_f, cb_capture_0);
	if (res == nullptr)
	{
		lua_pushnil(L);
	}
	else
	{
		xlua_pushuserdata<XPLMHotKeyID>(L, res);
	}

	return 1;
}

int XLuaUnregisterHotKey(lua_State* L)
{
	XPLMHotKeyID inHotKey = {};
	if (lua_isuserdata(L, 1))
	{
		inHotKey = xlua_checkuserdata<XPLMHotKeyID>(L, 1, "Expected userdata<XPLMHotKeyID>");
	}

	XPLMUnregisterHotKey(inHotKey);

	return 0;
}

int XLuaCountHotKeys(lua_State* L)
{
	int res = XPLMCountHotKeys();
	lua_pushinteger(L, res);

	return 1;
}

int XLuaGetNthHotKey(lua_State* L)
{
	int inIndex = luaL_checkinteger(L, 1);

	XPLMHotKeyID res = XPLMGetNthHotKey(inIndex);
	if (res == nullptr)
	{
		lua_pushnil(L);
	}
	else
	{
		xlua_pushuserdata<XPLMHotKeyID>(L, res);
	}

	return 1;
}

int XLuaGetHotKeyInfo(lua_State* L)
{
	XPLMHotKeyID inHotKey = {};
	if (lua_isuserdata(L, 1))
	{
		inHotKey = xlua_checkuserdata<XPLMHotKeyID>(L, 1, "Expected userdata<XPLMHotKeyID>");
	}
	char outVirtualKey[1];
	XPLMKeyFlags outFlags;
	char outDescription[512];
	XPLMPluginID outPlugin;

	XPLMGetHotKeyInfo(inHotKey, outVirtualKey, &outFlags, outDescription, &outPlugin);

	lua_createtable(L, 0, 4); // 0 array slots and 4 key-value pairs

	lua_pushstring(L, "outVirtualKey");
	lua_pushstring(L, outVirtualKey);
	lua_settable(L, -3);

	lua_pushstring(L, "outFlags");
	lua_pushinteger(L, outFlags);
	lua_settable(L, -3);

	lua_pushstring(L, "outDescription");
	lua_pushstring(L, outDescription);
	lua_settable(L, -3);

	lua_pushstring(L, "outPlugin");
	xlua_pushuserdata<XPLMPluginID>(L, outPlugin);
	lua_settable(L, -3);

	return 1;
}

int XLuaSetHotKeyCombination(lua_State* L)
{
	XPLMHotKeyID inHotKey = {};
	if (lua_isuserdata(L, 1))
	{
		inHotKey = xlua_checkuserdata<XPLMHotKeyID>(L, 1, "Expected userdata<XPLMHotKeyID>");
	}
	std::string inVirtualKey_s = luaL_checkstring(L, 2);
	if (inVirtualKey_s.size() != 1)
	{
		luaL_argerror(L, 2, "inVirtualKey must be exactly one character");
		return 0;
	}
	char inVirtualKey = inVirtualKey_s[0];
	XPLMKeyFlags inFlags = luaL_checkinteger(L, 3);

	XPLMSetHotKeyCombination(inHotKey, inVirtualKey, inFlags);

	return 0;
}

#ifdef __cplusplus
}
#endif

#endif
