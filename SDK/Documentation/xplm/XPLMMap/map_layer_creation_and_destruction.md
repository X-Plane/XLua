...
| mapToCreateLayerIn | const char * | Globally unique string identifying the map you want this layer to appear in. As of XPLM300, this is limited to one of XPLM_MAP_USER_INTERFACE or XPLM_MAP_IOS |
...
| showUiToggle | int | True if you want a checkbox to be created in the map UI to toggle this layer on and off; false if the layer should simply always be enabled |
| layerName | const char * | Short label to use for this layer in the user interface |
| refcon | void* | A reference to arbitrary data that will be passed to your callbacks |
