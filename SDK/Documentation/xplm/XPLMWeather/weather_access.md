...
| wind_layers[XPLM_NUM_WIND_LAYERS] | XPLMWeatherInfoWinds_t | Defined wind layers. Not all layers are always defined. |
| cloud_layers[XPLM_NUM_CLOUD_LAYERS] | XPLMWeatherInfoClouds_t | Defined cloud layers. Not all layers are always defined. |
| temp_layers[XPLM_NUM_TEMPERATURE_LAYERS] | float | Temperatures at altitude, in degrees C. Layer altitudes are the same globally - see the 'sim/weather/region/atmosphere_alt_levels_m' dataref. |
| dewp_layers[XPLM_NUM_TEMPERATURE_LAYERS] | float | Dewpoints at altitude, in degrees C. Layer altitudes are the same globally - see the 'sim/weather/region/atmosphere_alt_levels_m' dataref. |
